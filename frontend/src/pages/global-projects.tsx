        ...
        <div className="min-h-screen bg-[#111110] text-stone-100">
            <div className="bg-[#1C1C1A] border-b border-stone-800 px-6 py-6">
                <div className="max-w-5xl mx-auto">
                    <button onClick={() => router.back()} className="text-xs text-stone-500 hover:text-amber-400 mb-4 block transition-colors">
                        ← Back
                    </button>
                    <h1 className="text-2xl font-semibold tracking-tight text-stone-100">{project.title}</h1>
                    <p className="text-stone-500 text-sm mt-1">by {project.ownerName}</p>
                </div>
            </div>
                ...
                <div className="grid md:grid-cols-3 gap-6">
                    ...
                    <div className="md:col-span-2 space-y-4">
                        ...
                        <div className="bg-[#1C1C1A] rounded-2xl border border-stone-800 p-6">
                            {project.description && <p className="text-stone-400 text-sm leading-relaxed mb-4">{project.description}</p>}
                            {project.usedTechs && (
                                <div className="flex flex-wrap gap-1.5 mb-4">
                                    {project.usedTechs.split(',').map((tech, idx) => (
                                        <span key={idx} className="text-xs bg-amber-950/40 text-amber-400 border border-amber-800/50 px-2 py-0.5 rounded-lg">{tech.trim()}</span>
                                    ))}
                                </div>
                            )}
                            {project.githubLink && (
                        ...
                        {/* Rate form */}
                        <div id="rate-form" className="bg-[#1C1C1A] rounded-2xl border border-stone-800 p-6">
                            ...
                            <form onSubmit={handleSubmitRating}>
                                    ...
                                    <div className="flex gap-1 mb-2">
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <button key={star} type="button"
                                                    ...
                                                    className={`text-3xl transition-colors ${star <= (hovered || rating) ? 'text-amber-400' : 'text-stone-700'}`}>
                                                ★
                                            </button>
                                        ))}
                                    </div>
                                    <p className="text-xs text-stone-500">{rating} of 5 stars</p>
                                </div>
                                ...
                                />
                                <p className="text-xs text-stone-600 mb-4">{comment.length}/500</p>
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {isEditing && (
                                        <button type="button" onClick={handleDeleteRating}
                                                className="bg-stone-800 text-red-400 border border-stone-700 px-5 py-2 rounded-xl text-sm font-medium hover:bg-red-950/40 hover:text-red-300 transition-colors">
                                            Delete
                                        </button>
                                    )}
                                </div>
                        ...
                        <div className="bg-[#1C1C1A] rounded-2xl border border-stone-800 p-6">
                            ...
                            {ratings.length === 0 ? (
                                <p className="text-stone-600 text-sm italic">No reviews yet. Be the first!</p>
                            ) : (
                                <div className="space-y-4">
                                    {ratings.map((r) => (
                                        <div key={r.id} className="border-b border-stone-800/60 pb-4 last:border-0 last:pb-0">
                                            <div className="flex items-center justify-between mb-1">
                                                <p className="text-sm font-medium text-stone-200">{r.userName}</p>
                                                <span className="text-xs text-stone-600">{new Date(r.createdAt).toLocaleDateString()}</span>
                                            </div>
                                            <div className="text-sm mb-1">
                                                <span className="text-amber-400">{'★'.repeat(r.rating)}</span>
                                                <span className="text-stone-700">{'★'.repeat(5 - r.rating)}</span>
                                            </div>
                                            {r.comment && <p className="text-stone-400 text-sm leading-relaxed">{r.comment}</p>}
                                        </div>
                                    ))}
                                </div>
                            )}
                    ...
                    {/* Right: Summary */}
                    <div>
                        <div className="bg-[#1C1C1A] rounded-2xl border border-stone-800 p-6 sticky top-6">
                            <p className="text-xs text-stone-500 uppercase tracking-widest mb-4">Summary</p>
                            <div className="text-center mb-6 pb-6 border-b border-stone-800">
                                <p className="text-5xl font-semibold text-amber-400 mb-1">
                                    {project.averageRating?.toFixed(1) || '—'}
                                </p>
                                <div className="text-lg mb-1">
                                    <span className="text-amber-400">{'★'.repeat(Math.round(project.averageRating || 0))}</span>
                                    <span className="text-stone-700">{'★'.repeat(5 - Math.round(project.averageRating || 0))}</span>
                                </div>
                                <p className="text-xs text-stone-500">{project.totalRatings} rating{project.totalRatings !== 1 ? 's' : ''}</p>
                            </div>
                                ...
                                {[5, 4, 3, 2, 1].map((star) => {
                                    ...
                                    return (
                                        <div key={star} className="flex items-center gap-2 text-xs">
                                            ...
                                            <span className="text-stone-600 w-4 text-right">{count}</span>
                                        </div>
                                    );
                                })}
                            </div>
